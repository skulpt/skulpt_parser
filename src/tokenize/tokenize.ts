...
const Pointfloat =
  group("[0-9](?:_?[0-9])*\\.(?:[0-9](?:_?[0-9])*)?", "\\.[0-9](?:_?[0-9])*") +
  maybe(Exponent);
...
const Special = group(
  ...EXACT_TOKENS_SORTED.reverse().map((t) => regexEscape(t)),
);
const Funny = group("\\r?\\n", Special);
...
const PseudoToken = Whitespace +
  group(PseudoExtras, Number_, Funny, ContStr, Name);
...
function* _tokenize(
  ...
  while (true) {
    ...
    if (contstr) {
      ...
      if (endmatch) {
        ...
        yield new TokenInfo(tokens.ERRORTOKEN, contstr + line, strstart, [
          lnum,
          line.length,
        ], contline as string);
        contstr = "";
      ...
      if ("#\r\n".includes(line[pos])) {
        ...
        yield new TokenInfo(tokens.NL, line.substring(pos), [lnum, pos], [
          lnum,
          line.length,
        ], line);
        continue;
      ...
      if (column > indents[indents.length - 1]) {
        ...
        indents.push(column);
        yield new TokenInfo(tokens.INDENT, line.substring(pos), [lnum, 0], [
          lnum,
          pos,
        ], line);
      }
      ...
      if (pseudomatch !== null) {
        ...
        if (
          ...
          if (token[token.length - 1] === "\n") {
            ...
            endprog = RegExp(
              endpats[initial] || endpats[token[1]] || endpats[token[2]],
            );
            contstr = line.substring(start);
            needcont = 1;
            contline = line;
        ...
        yield new TokenInfo(tokens.ERRORTOKEN, line[pos], [lnum, pos], [
          lnum,
          pos + 1,
        ], line);
        pos += 1;
  ...
  if (lasline && !"\r\n".includes(lasline[lasline.length - 1])) {
    yield new TokenInfo(tokens.NEWLINE, "", [lnum - 1, lasline.length], [
      lnum - 1,
      lasline.length + 1,
    ], "");
  }
